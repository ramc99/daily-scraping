...
async def scrape_all(headless: bool = True, limit: int = 0, city: str = ""):
    """
    Reads all {city}_phase1.json files from phase1/ and runs Phase 2 per city.
    Pass city= to process a single city only.
    Pass limit= to cap facilities (for testing).
    """
    ...
    for phase1_file in phase1_files:
        ...
        city_results = await scrape_city(None, city_s, query_s, unique, headless)
        all_results.extend(city_results)

    log.info("All cities done. Total: %d facilities.", len(all_results))
    return all_results
...
async def main(headless: bool = True, limit: int = 0, city: str = ""):
    await scrape_all(headless=headless, limit=limit, city=city)
